...
@app.route(prefix + version + endpoint + 'package-names', methods=['GET'])
def get_package_names():
    ...
    packages = qa_lib.get_package_names(folder)
    results = dict(packages=packages)
...
def get_uri_txt():
    ...
    uri = qa_lib.get_uri_txt(folder, package)

    results = dict(uri_results=uri)
...
@app.route(prefix + version + endpoint + 'package-file-count', methods=['GET'])
def get_package_file_count():
    """
    Runs QA process to package file count
    @param: api_key
    @param: folder
    @returns: Json
    """
    ...
    file_count = qa_lib.get_package_file_count(folder, package)

    results = dict(file_count=file_count)
...
def move_to_ingest():
    ...
    results = qa_lib.move_to_ingest(uuid, folder, package)
